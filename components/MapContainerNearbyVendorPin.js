  ...
  componentDidMount(){
	...
	onMapClick(t, map, coord){
		...
		if(this.state.showing_info_box){
			...
			this.setState({showing_info_box: false,
						active_marker: {}});
		}
	...
	render(){
		...
		<>
			...
			>
			{this.state.user_marker.position != null ? <Marker title = {this.state.user_marker.title}
					position = {{lat: this.state.user_marker.position.lat, lng: this.state.user_marker.position.lng}}
			...
			{Array.from(this.state.nearby_vendors).map((single_vendor) => ( //The found nearby vendors
				<Marker
		  ...
		  {this.state.showing_info_box ? (<InfoWindowEX position={{ lat: this.state.active_marker.lat, lng: this.state.active_marker.lng }} 
						visible={this.state.showing_info_box} 
				onClick={() => console.log("Power Pizza")}
				onCloseClick={() => {this.setState({active_marker: {},
													selected_location: {},
													showing_info_box: false})
			}}>
				<div>
