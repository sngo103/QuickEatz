...
export class WriteReview extends React.Component {
  ...
  componentDidMount(){
	...
	fetch(`/api/getVendorName?_id=${Router.query.vendor_id}`)//Get the business name of the reviewee for readability
															...
															.then(v_json => {this.setState({current_vendor_name: v_json.business_name})
  ...
  componentDidUpdate(){
	  if(this.state.current_user == "" || this.state.isLoggedIn != null){
		  ...
		  if(this.state.current_user == ""){ //Failed to get token
  ...
  handleSubmit(event) {
    ...
    const data = fetch(
      `/api/sendReview?user_email=${user_id_str}&vendor_id=${vendor_id_str}&review_content=${rev_info.review_text}&rating=${rev_info.review_rating}`
			...
			if(response.status != 400){ //Im probably misusing the convention, but Ill leave it as 400 for now. See sendReview.
  ...
  render(){
    ...
    <>
	  ...
	  <h1 className="container p-3 text-5xl">{this.state.current_vendor_name}</h1>
        <br />
  ...
  <select className="text-xl border-2 border-black rounded-md p-1" id="rating" value={this.state.review_rating} onChange= {this.handleChange} required>
